...
export function useVssueStore() {
  ...
  const VssueState = reactive<VssueState>({
    options: null,
    title: import.meta.env.VITE_COMMENT_TITLE,
    issueId: null,
    API: null,
    accessToken: null,
    user: null,
  ...
  const version = computed((): string => pkgVersion)
  ...
  function setOptions(options: Partial<Vssue.Options>) {
    VssueState.options = Object.assign(
      {
        labels: import.meta.env.VITE_COMMENT_LABELS.split(','),
        state: import.meta.env.VITE_COMMENT_STATE,
        prefix: import.meta.env.VITE_COMMENT_PREFIX,
        admins: [],
        perPage: 10,
        proxy: (url: string): string =>
          `${import.meta.env.VITE_COMMENT_PROXY}${url}`,
